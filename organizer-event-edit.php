...
<head>
	...
	function showUser(){
		var ajaxRequest;  // The variable that makes Ajax possible!
		 ...
		 queryString +=  "&electurer=" + electurer + "&ehour=" + ehour + "&ehour_to=" + ehour_to + "&event_id=" + event_id;
		 
		 ajaxRequest.open("GET", "ajax-getLecturerforedit.php" + 
	                              		queryString, true);	
...
<div id="wrapper">
	...
	<div id="maincontentcontainer" class="s_contact">
		<div id="maincontent">
			...
			<div class="breadcrumb">
            	<a href="index.php">Home</a> &raquo; <a href="organizer-panel.php">Organizer</a> &raquo; Edit <?php echo $details->ename ?>
            </div>
			...
			<div class="section group">
                ...
                <div class="col span_3_of_5">              	
                	<h3 style="text-align:center;background:#42B8DD"><span style="color:#fff" class="contactJobtitle">Edit <?php echo $details->ename ?></span></h3>
            		<div class="section group" style="background:#DDD;">
	            		<div class="col span_4_of_4">
	            			<div style="padding:0 10px;width:93%;margin:0 auto;">
								<form id="eventUpdateForm" action="" method="post" enctype="multipart/form-data">									
			                        
			                        <label for="">Event Name <i>*</i></label>  
			                        <input type="text" id="ename" name="ename" value="<?php echo $details->ename ?>" required placeholder="Please enter event name">
			                        ...
			                        <select required name="ecategory_id" id="ecategory_id">
			                        	...
			                        	<?php 
											$role = new User();
											echo array2dropdown($role->getSubjectDropDown(),$details->ecategory_id);
										?>
			                        </select> 
			                        <img style="display:none;" id="jaximages" src="img/loading.gif" />
			                        ...
			                        <div class="section group">
			                        	<div class="col span_1_of_6">
			                        		<input type="date" id="edate" onchange="showUser()" name="edate" value="<?php echo $details->edate ?>" class="datepicker" required placeholder="Please enter date">
			                        	</div>
			                        	<div class="col span_1_of_6" onchange="showUser()">
			                        		<select class="hour_24" style="" required name="ehour" id="ehour">
			                        	...
			                        	<div class="col span_1_of_6">    
					                        <select class="hour_24" style="" required name="eminute">
					                        	<option value="">Minute</option>
					                        	<?php 
													for ($i=0; $i<=59; $i++){
														if($i == $details->eminute){
															if($i < 10){
																echo'<option selected="selected" value="0'.$i.'">0'.$i.'</option>';
															}
															else{
			                        	...
			                        	<div class="col span_1_of_6" onchange="showUser()">
			                        		<select class="hour_24" style="" required name="ehour_to" id="ehour_to">
					                        	<option value="">Hour</option>
					                        	<?php 
													for ($i=1; $i<=24; $i++){
														if($i == $details->ehour_to){
															if($i < 10){
																echo'<option selected="selected" value="0'.$i.'">0'.$i.'</option>';
															}
															else{
			                        	...
			                        	<div class="col span_1_of_6">   
			                        		<select class="hour_24" style="" required name="eminute_to">
					                        	<option value="">Minute</option>
					                        	<?php 
													for ($i=0; $i<=59; $i++){
														if($i == $details->eminute_to){
															if($i < 10){
																echo'<option selected="selected" value="0'.$i.'">0'.$i.'</option>';
															}
															else{
			                        	...
			                        	<div style="float:left;" id="ajaxDiv"></div>
			                        </div>	
			                        ...
			                        <select required name="elocation" id="elocation">
			                        	...
			                        	<?php 
			                        		$role = new User();
			                        		echo array2dropdown($role->getLocationDropDown(), $details->elocation_id);
			                        	?>
			                        </select>
			                        ...
			                        <label style="margin-top:10px">Multimedia <i>*</i></label>  
			                        <select required name="multimedia[]" id="multimedia" multiple class="chosen-select" tabindex="2">
			                        	<?php 
			                        		$role = new User();

			                        		$getRole = $role->getMultimedia();

			                        		if ($getRole) {

			                        			foreach($getRole as $single){

													$single = (object)$single;

													$Type_exploded = explode(",", $details->emultimedia);
													
													if(in_array($single->id, $Type_exploded))  {
														echo "<option value=" . $single->id . " selected>" . $single->name . "</option>";
													}
													else{
			                        ...
			                        <label style="margin-top:10px">Eligibility Criteria <i>*</i></label>  
			                        <select required name="eligibility_id">
			                        	<option value="">Select Eligibility</option>
			                        	<?php 
											$role = new User();
											echo array2dropdown($role->getEligibilityDropDown(),$details->eligibility_id);
										?>
			                        </select>

			                        <label style="margin-top:10px">Supporters <i>*</i></label>  
			                        <select required name="supporters[]" id="supporters" multiple class="chosen-select" tabindex="2">
			                        	<?php 
			                        		$role = new User();

			                        		$getRole = $role->getSupporters();
			                        		...
			                        		if ($getRole) {
			                        			...
			                        			foreach($getRole as $single){
													...
													$Type_exploded = explode(",", $details->esupporters);
													
													if(in_array($single->id, $Type_exploded))  {
														echo "<option value=" . $single->id . " selected>" . $single->fname . " " . $single->lname . "</option>";
													}
													else{
														echo "<option value=" . $single->id . ">" . $single->fname . " " . $single->lname . "</option>";
													}
												}
			                        		}
			                        	?>
                        			...
                        			<textarea id="edescription" name="edescription" tabindex="5" required><?php echo $details->edescription ?></textarea></p>                  	                       

                        			<label for="">Organized by <i>*</i></label>  
			                        <input type="text" id="o_by" name="o_by" required value="<?php echo $details->o_by ?>" placeholder="">
                        			...
                        			<input type="hidden" value="<?php echo $details->id ?>" name="eid" id="eid">
                        			<input type="hidden" value="<?php echo $details->evenue_no ?>" name="venue_name" id="venue_name">
                        			<input type="hidden" value="<?php echo $details->organizer_id ?>" name="organizer_id" id="organizer_id">
			                        <button type="submit" name="UpdateEvent" id="UpdateEvent" class="button-secondary pure-button">Update</button>
			                    </form>  
			                </div>      
            			</div>
            		</div>  
	...
	$(document).ready(function(){
		...
		$.ajax({ 
			type:'post',
			url: 'ajax-getEditVenue.php',
			data: {udata: postdata,eventid: eventid},
	...
	$(document).ready(function(){
		...
		$('#jaximages').fadeIn("slow");
		$('#loadLecturer').css("display", "none");
		$.ajax({ 
			type:'post',
			url: 'ajax-getEditLecturerDropdown.php',
			data: {udata: postdata,eventid: eventid},
			success: function(data){ 
				$('#jaximages').css("display", "none");
	...
	<script>
		$(document).ready(function(){		  
			...
			$.ajax({ 
				type:'GET',
				url: 'ajax-getSeats.php',
				data: {venue_name: venue_name},
				success: function(data){ 
					$('#ajaxDiv2').html(data).fadeIn("slow"); 
				}
			});	
		});
	</script>
	...
	<script src="ajax-js-files/event-edit-validation.js"></script>
